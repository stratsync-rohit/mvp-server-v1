...
class SlackDestinationService:
    ...
    @staticmethod
    def _parse_channel_link(
        channel_link: str
    ) -> tuple[str, str, str]:

        normalized_link = channel_link.strip()

        parsed = parse_slack_channel_link(
            normalized_link
        )
    ...
    async def create_destination(
        ...
        normalized_webhook = self._validate_webhook(
            webhook_url
        )
        ...
        try:
            return (
                await self.slack_destination_repository.create_destination(
                    client_id=client_id,
    ...
    async def get_destinations_by_client(
        self,
        client_id: str
    ):
    ...
    async def update_destination(
        self,
        destination_id: str,
        update_data: SlackDestinationUpdate
    ):

        if not ObjectId.is_valid(destination_id):
            raise ValueError(
                "Invalid Slack destination ID"
            )
        ...
        if "member_name" in updates:
            ...
            if updates["member_name"] is None:
                raise ValueError(
                    "Invalid member name"
                )

            updates["member_name"] = (
                updates["member_name"].strip()
            )

            if not updates["member_name"]:
                raise ValueError(
                    "Invalid member name"
                )
        ...
        # ------------------------------------------------------
        # Duplicate active destination check
        # ------------------------------------------------------

        target_active = updates.get(
            "is_active",
            existing.get(
                "is_active",
                True
    ...
    async def delete_destination(
        ...
        await self.get_destination_by_id(
            destination_id
        )

        deleted = (
            await self.slack_destination_repository.soft_delete(
                destination_id
            )
        )
    ...
    async def test_destination(
        ...
        if self.slack_n8n_service is None:
            raise RuntimeError(
                "Slack notification service is not configured"
            )

        # ------------------------------------------------------
        # n8n flow stays unchanged.
        # member_name is only additional destination metadata.
        # ------------------------------------------------------
        ...
        await self.slack_n8n_service.trigger_notification(
            payload
        )