...
def _service(
    ...
    return SlackDestinationService(
        slack_destination_repository=SlackDestinationRepository(
            database
        ),
...
async def create_slack_destination(
    ...
    try:
        ...
        logger.exception(
            "slack_destination_create_failed "
            "client_id=%s "
...
async def update_slack_destination(
    ...
    try:
        ...
        logger.exception(
            "slack_destination_update_failed "
            "destination_id=%s "
            "error_code=unexpected_error",
            destination_id,
        )
...
async def delete_slack_destination(
    ...
    try:
        ...
        logger.exception(
            "slack_destination_delete_failed "
            "destination_id=%s "
            "error_code=unexpected_error",
            destination_id,
        )
...
async def test_slack_destination(
    destination_id: str,
    database=Depends(get_database),
    slack_n8n_service=Depends(
        get_slack_n8n_service
    ),
):
    try:
        # Slack test notification continues through n8n.
        await _service(
            database,
            slack_n8n_service,
        ).test_destination(
